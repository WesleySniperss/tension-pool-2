import { getSetting, setSetting, isGM } from "./constants.js";
...
export interface TensionPoolAPI {
  ...
  roll(): Promise<TensionRollResult | undefined>;
  clear(): Promise<void>;
  customRoll(count: number): Promise<TensionRollResult | undefined>;
  getDiceCount(): number;
  getPoolSize(): number;
}
...
/**
 * The pool lives in world-scoped settings, which only a GM may write. Players get
 * a clear notification instead of a raw "lacks permission" exception from the
 * example macros or any other API caller.
 */
function requireGM(): boolean {
  if (isGM()) return true;
  (ui as any).notifications?.warn(game.i18n!.localize("VTP.GMOnly"));
  return false;
}
...
export function createTensionPoolAPI(): TensionPoolAPI {
  ...
  return {
    ...
    async roll(): Promise<TensionRollResult | undefined> {
      if (!requireGM()) return;
      const current = getSetting("diceCount");
      const max = getSetting("poolSize");
    ...
    async customRoll(count: number): Promise<TensionRollResult | undefined> {
      if (!requireGM()) return;
      count = sanitize(count);
      const max = getSetting("poolSize");
