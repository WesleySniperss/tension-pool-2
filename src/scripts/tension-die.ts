...
export async function rollTensionPool(diceCount: number): Promise<TensionRollResult> {
  ...
  const icons = hasComplication
    ? results
        .filter((r) => r === 1)
        .map(() => '<i class="vtp-die vtp-die-hit" data-vtp-icon="tension"></i>')
        .join("")
    : '<i class="vtp-die vtp-die-miss" data-vtp-icon="noTension"></i>';

  const outcome = hasComplication
    ? `<strong class="vtp-result-hit">${i18n.localize("VTP.Complication")}</strong>`
    : `<strong class="vtp-result-safe">${i18n.localize("VTP.Safe")}</strong>`;
